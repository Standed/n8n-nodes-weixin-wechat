...
> ✨ **v1.2.21新版**：优化一键启动体验！解决首次运行需要二次点击的问题，现在真正实现一键式自动化部署，用户体验大幅提升！
...
### v1.2.21 (最新) 🎉
- 🚀 **一键启动优化**：彻底解决首次运行需要二次点击的用户体验问题
- 🔧 **流程改进**：移除依赖安装后的不必要暂停，实现真正的一键自动化部署
- 📋 **用户引导**：新增首次运行说明，明确告知用户整个过程是自动化的，预期耗时1-2分钟
- ✅ **体验提升**：从"安装→按键→退出→再次点击"优化为"一次点击→自动完成所有步骤"
- 🎯 **无缝部署**：Windows用户现在可以真正享受无中断的一键部署体验
...
### v1.2.20
- 🎉 **完整文件发送**：个人微信文件发送功能全面实现，支持URL文件和二进制文件发送
- 🌐 **URL文件支持**：自动下载网络文件并发送，智能识别文件类型和扩展名
- 💾 **二进制数据处理**：完美支持N8N二进制数据格式，自动base64解码和文件保存
- 🔧 **智能文件处理**：安全文件名处理、MIME类型检测、临时文件自动清理
- ✅ **云端部署修复**：解决云端N8N环境文件发送失败的问题，现已完全支持
